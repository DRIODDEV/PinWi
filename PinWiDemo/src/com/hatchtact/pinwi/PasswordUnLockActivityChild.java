...
public  class PasswordUnLockActivityChild extends MainActionBarActivity 
{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		typeFace= new TypeFace(PasswordUnLockActivityChild.this); 

		sharePrefclass=new SharePreferenceClass(PasswordUnLockActivityChild.this);
		showMessage = new ShowMessages(PasswordUnLockActivityChild.this);
		...
		button_cancel.setOnClickListener(
				new OnClickListener() {
					...
					public void onClick(View arg0)
					{
						...
						if(loadNextScreen)
						{
							PasswordUnLockActivityChild.this.finish();
							Intent intent=new Intent(PasswordUnLockActivityChild.this, AccessProfileActivity.class);
	...
	protected void onPinLockInserted() {
		...
		if(StaticVariables.childPasscodeList!=null && StaticVariables.childPasscodeList.getPasscodeList().size()>0)
		{
			for(int i=0;i<StaticVariables.childPasscodeList.getPasscodeList().size();i++)
			{
				if(StaticVariables.childPasscodeList.getPasscodeList().get(i).getProfileId()==profileId && 
						passLock.trim().equalsIgnoreCase(StaticVariables.childPasscodeList.getPasscodeList().get(i).getPassCode().trim()))
				{
					passCodeMatched = true;
					ProfileType = StaticVariables.childPasscodeList.getPasscodeList().get(i).getPassCodeType();
					break;
				}
			}
		}
		...
		if(passCodeMatched)
		{
			...
			{	
				if(loadNextScreen)
				{
					...
					Intent intent=new Intent(PasswordUnLockActivityChild.this, ChildTutorialActivity.class)	;
					startActivity(intent);
				}
				else
				{
			...
			/*else
			{
				...
				{
					AccessProfileActivity.getInstance().finish();
					Intent intent=new Intent(PasswordUnLockActivityChild.this, ParentProfileInformationActivity.class)	;
					startActivity(intent);
				}
	...
	private class SendPasscodeOnEmail extends AsyncTask<Void, Void, Integer>
	{
		...
		protected void onPreExecute() {
			...
			super.onPreExecute();
			progressDialog = ProgressDialog.show(PasswordUnLockActivityChild.this, "", StaticVariables.progressBarText, false);
		...
		protected Integer doInBackground(Void... params) {
			...
			if(checkNetwork.checkNetworkConnection(PasswordUnLockActivityChild.this))
			{
				ErrorCode=serviceMethod.recoverPasscodeChild(profileId);
			}
			else
			{
				ErrorCode=-1;
